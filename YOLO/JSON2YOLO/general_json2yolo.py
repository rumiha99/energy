...
def convert_coco_json(json_dir="../", use_segments=False, cls91to80=False):
    ...
    save_dir = make_dirs(dir=f"/home/YOLO/{folder_name}/datasets")  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    for json_file in sorted(Path(json_dir).resolve().glob("*.json")):
        fn = Path(save_dir) / "labels" / json_file.stem.replace("", "")  # folder name
        fn.mkdir()
...
if __name__ == "__main__":
    ...
    if source == "COCO":
        convert_coco_json(
            f"/home/YOLO/{folder_name}",  # directory with *.json
            use_segments=True,  #最初のほうのとこも変えてね
            cls91to80=True,
        )
